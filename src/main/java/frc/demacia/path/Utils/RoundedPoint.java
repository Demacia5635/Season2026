...
public class RoundedPoint {
    ...
    public RoundedPoint(PathPoint aPoint, PathPoint bPoint, PathPoint cPoint) {
        ...
        heading1 = bPoint.getRotation().getRadians();
        ...
        if (Math.abs(cornerAngle.getDegrees()) >= 175 && Math.abs(cornerAngle.getDegrees()) <= 180)
            ...
            if (radius > getMaxRadius()) {
                radius = getMaxRadius();
                // //System.out.println("radius is bigger then possible, new radius is: " +
                // radius);
            }
    ...
    public double getWantedVelocity1() {
    ...
    /**
     * 
     * @return The starting position of the corner's curve (relative to the corner's
     *         circle's center)
     */
