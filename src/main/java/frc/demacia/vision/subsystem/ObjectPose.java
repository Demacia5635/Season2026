...
public class ObjectPose extends SubsystemBase {
  ...
  /**
   * Constructor - Initializes the object tracker with camera configuration and
   * robot position suppliers.
   ...
   * Reads the latest vision data (pitch, yaw) from NetworkTables and updates the
   * object's field position
  ...
  public void periodic() {
    ...
    isSee = Table.getEntry("tv").getDouble(0.0) != 0;
    if (isSee) {
   ...
   * Uses trigonometry with the camera height, mount angle, and target angle to
  ...
  /**
   * Calculates the translation vector from the robot's center to the detected
   * object.
   * First calculates camera-to-object translation, then adds the camera's offset
   * from robot center.
   * 
  ...
  /**
   * Calculates the translation vector from the field origin to the detected
   * object.
   * Rotates the robot-to-object vector by the robot's field angle, then adds the
