...
public class DriveToFuel extends Command {
  ...
  public DriveToFuel(Chassis chassis, CommandController controller, IntakeSubsystem intake, ObjectPose objectPose) {
  ...
  public void execute() {
    ...
    if (ChassisSpeeds.fromFieldRelativeSpeeds(speeds, chassis.getGyroAngle()).vxMetersPerSecond > 0 && objectPose.isSee()) {
      intake.setDutyIntake(0.8);
      chassis.followBall(objectPose, speeds);
    } else {
